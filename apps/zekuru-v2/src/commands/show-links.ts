...
import { IChannelLink, ITranslateChannel } from '@zekuru-v2/types';
import {
...
const execute = async (interaction: ChatInputCommandInteraction) => {
  ...
  if (channel) {
    const [chLink, trChannel] = await Promise.all([
      channelLinkCache.get(channel.id),
      translateChannelCache.get(channel.id),
    ]);
    if (!chLink || !trChannel) {
      interaction.reply({
  ...
  const strBuilder = await Promise.all<string>(
    chLinks.map(async (chLink, i) => {
      const trChannel = await translateChannelCache.get(chLink.id);
      if (!trChannel) return '';
